            ...
            for sentence in doc:
                for token in sentence:
                    ...
                    if idVals == None:
                        ...
                        idVals[id] = freqVal + 1
                        if(max_freq < idVals[id]):
                            max_freq = idVals[id]
            max_freq_doc[id] = max_freq
        for type in index:
            item0 = index[type]
            for doc_id in item0:
                index[type][doc_id] = index[type][doc_id] / max_freq_doc[doc_id]
        ...
        for query in queries:
            ...
            for sentence in query:
                for token in sentence:
                    ...
                    if(max_freq < query_tokens[token]):
                        max_freq = query_tokens[token]
            for token in query_tokens:
                query_magnitude += ((0.5 + 0.5*query_tokens[token]/max_freq)*idf)**2
